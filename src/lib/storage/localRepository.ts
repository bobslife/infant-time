...
export async function createLocalEvent(input: CreateEventInput): Promise<BabyEvent> {
  const nextEvent: BabyEvent = {
    ...
    medicineDose: input.medicineDose ?? null,
    medicineNextAt: input.medicineNextAt ? new Date(input.medicineNextAt).toISOString() : null,
    temperatureC: input.temperatureC ?? null,
    temperatureLocation: input.temperatureLocation ?? null,
    mealStage: input.eventType === "meal" ? input.mealStage ?? null : null,
    mealToppings: input.eventType === "meal" ? input.mealToppings ?? [] : [],
    mealName: input.mealName ?? null,
    mealAmountG: input.mealAmountG ?? null,
    mealReaction: input.mealReaction ?? null,
    note: input.note,
    createdAt: new Date().toISOString(),
  };
